...
if (!dutySource.includes(".duty-row>*{min-width:0}") || !dutySource.includes("grid-template-columns:minmax(0,1fr)") || !dutySource.includes("#duty-date::-webkit-date-and-time-value") || !dutySource.includes("width:calc(100% - 24px)!important") || !dutySource.includes("duty-date-field")) {
...
if (!commonSource.includes("pendingTeamSort = true") || !commonSource.includes("pendingTeamSort && sortTeamRowsForActiveDate()")) {
  fail("common.js", "回答タップ中の行移動を防ぐ遅延並び替えがありません");
}
if (!guideSource.includes("次の日程へ切り替えた時に並び替えます") || !guideSource.includes("チーム名の読み順")) {
  fail("guide.html", "試合調整の遅延並び替えが説明されていません");
}
...
if (coachSource.includes("function renderAttention") || coachSource.includes("data-coach-attention-id")) {
  fail("coach.html", "削除した大会一覧の要対応表示が残っています");
}
...
if (!guideSource.includes("<strong>試合・大会</strong>") || !guideSource.includes("同日開催の複数大会も別々に選べます")) {
  fail("guide.html", "コーチ専用フォルダの大会一覧タブが説明されていません");
}
if (!guideSource.includes("準備完了数") || !guideSource.includes("直近3大会") || !guideSource.includes("大会準備の状況をLINEで共有")) {
  fail("guide.html", "試合・大会内の縦型大会一覧とLINE共有が説明されていません");
}
...
if (!guideSource.includes("未保存を含む全日程") || !guideSource.includes("直近3大会")) {
